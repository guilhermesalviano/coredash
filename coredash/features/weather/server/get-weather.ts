...
export async function getWeatherData(limit?: number): Promise<WeatherData> {
  ...
  const cacheKey = [limit ?? 10, settings.settings.latitude, settings.settings.longitude, settings.settings.timezone].join(":");
  const cached = weatherCache.get(cacheKey);
  if (cached) return cached;

  const weather = await withRetry(() => fetchOpenMeteoAPI({
  ...
  const forecast = weather.hourly.time.map((timestamp, index) => ({
    timestamp,
    time: new Date(timestamp).toLocaleTimeString("pt-BR", { hour: "2-digit", timeZone: settings.settings.timezone }) + "h",
